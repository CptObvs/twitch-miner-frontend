  ...
  template: `
    <div class="rounded-xl border border-gray-800 bg-gray-800/30">
      ...
      >
        ...
        <svg
          [class]="'h-4 w-4 text-gray-500 transition-transform duration-200 ' + (expanded() ? 'rotate-180' : '')"
          fill="none"
          viewBox="0 0 24 24"
          stroke-width="2"
          stroke="currentColor"
        ...
        <div class="border-t border-gray-700/50 px-3 pb-3 pt-2 space-y-2">
          ...
          @if (loading()) {
            <div class="flex justify-center py-3">
              <div class="h-4 w-4 animate-spin rounded-full border-2 border-gray-700 border-t-twitch"></div>
            </div>
            ...
            <div class="max-h-48 space-y-1.5 overflow-y-auto">
              @for (code of sortedCodes(); track code.id) {
                <div
                  [class]="'group flex items-center gap-2 rounded-lg px-2 py-1.5 text-xs ' +
                    (code.used_at ? 'bg-gray-900/50 opacity-50' : code.is_valid ? 'bg-gray-900/80' : 'bg-gray-900/50 opacity-50')"
                >
                  <div class="min-w-0 flex-1">
                    ...
                    <div class="mt-0.5 pl-3">
                      @if (code.used_at) {
                        <span class="text-gray-500">Used{{ code.used_by ? ' by ' + code.used_by : '' }}</span>
                      } @else if (code.is_valid) {
