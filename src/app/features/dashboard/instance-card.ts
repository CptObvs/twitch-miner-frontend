...
@Component({
  ...
  imports: [RouterLink, StatusBadge, UptimeTimer],
  template: `
    @if (compact()) {
      ...
      <div
        [class]="
          ...
          (instance().is_running
            ? 'border-twitch/20 bg-gray-900/80'
      ...
      >
        ...
        <div class="flex items-center">
          ...
          <span
            ...
            [attr.aria-label]="instance().is_running ? 'Running' : 'Stopped'"
          >
            <span
              [class]="
                'h-3 w-3 rounded-full ' +
                (instance().is_running ? 'bg-green-400 shadow-lg shadow-green-400/50' : 'bg-gray-500')
              "
            ></span>
          </span>
          <!-- Desktop: Full badge with text -->
          <app-status-badge [running]="instance().is_running" class="hidden sm:inline-flex" />
        </div>
        <div class="min-w-0 flex-1">
          ...
          @if (ownerName()) {
            <span class="block truncate text-xs text-gray-500">
              <svg class="inline-block h-3 w-3 mr-0.5 -mt-0.5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        <div class="flex items-center gap-1.5">
          @if (instance().is_running) {
            ...
            >
              @if (actionLoading()) {
                <span class="flex items-center gap-1.5">
                  <span class="h-3 w-3 animate-spin rounded-full border-2 border-red-300/30 border-t-red-300"></span>
                  Stopping
                </span>
              } @else {
                Stop
            ...
            >
              @if (actionLoading()) {
                <span class="flex items-center gap-1.5">
                  <span class="h-3 w-3 animate-spin rounded-full border-2 border-green-300/30 border-t-green-300"></span>
                  Starting
                </span>
              } @else {
                Start
        ...
        <div class="flex gap-3">
          @if (instance().is_running) {
            ...
            >
              @if (actionLoading()) {
                <span class="flex items-center justify-center gap-2">
                  <span class="h-4 w-4 animate-spin rounded-full border-2 border-red-300/30 border-t-red-300"></span>
                  Stopping...
                </span>
              } @else {
                Stop
            ...
            >
              @if (actionLoading()) {
                <span class="flex items-center justify-center gap-2">
                  <span class="h-4 w-4 animate-spin rounded-full border-2 border-green-300/30 border-t-green-300"></span>
                  Starting...
                </span>
              } @else {
                Start
