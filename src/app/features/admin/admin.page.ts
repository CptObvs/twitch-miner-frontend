  ...
  template: `
    <div class="mx-auto max-w-5xl">
      ...
      <div class="mb-6 relative">
        <div
          class="flex gap-1 rounded-xl bg-gray-900/50 backdrop-blur-sm border border-gray-800 p-1.5"
        >
          <button
      ...
      @if (activeTab() === 'users') {
        <div
          class="rounded-xl border border-gray-800 bg-gray-900/50 backdrop-blur-sm shadow-xl shadow-black/10 overflow-hidden"
        >
          @if (usersLoading()) {
            <app-loading-view />
          } @else {
            ...
            >
              ...
              @if (generatingCode()) {
                <span class="flex items-center gap-2">
                  <app-loading-spinner size="md" color="white" />
                  Generating...
                </span>
              } @else {
                <span class="flex items-center gap-2">
...
export class AdminPage {
  ...
  toggleUserRole({ userId, newRole }: { userId: string; newRole: Role }) {
    this.adminService
  ...
  updateInviteLimit({ userId, maxInviteCodes }: { userId: string; maxInviteCodes: number }) {
    this.adminService
