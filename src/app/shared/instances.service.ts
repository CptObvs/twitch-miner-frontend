...
type PointsSnapshot = components['schemas']['InstancePointsSnapshotResponse'];
...
export class InstancesService {
  ...
  updateStreamers$(id: string, streamers: string[]): Observable<string[]> {
    return defer(() =>
      ...
      map(({ data, error }) => {
        if (error || !data) throw new Error('Failed to update streamers');
        return data.streamers;
      }),
    );
  }

  getPointsSnapshot$(): Observable<PointsSnapshot[]> {
    return defer(() => from(api.GET('/instances/points-snapshot'))).pipe(
      map(({ data, error }) => {
        if (error || !data) throw new Error('Failed to load points snapshot');
        return data;
      }),
      catchError(() => of([])),
    );
